...
def populatin(count, length, min, max): 
...
def mulation(parents, mutate): 
    mulation_length = int(len(parents)*mutate)
    for _ in range(mulation_length):    
...
def GenaticAlgorithm(pop, target, retain=0.2, mutate=0.01):
    generation = 0
    found = False
    ...
    while (population[0][0] > 0): 
        ...
        new_generation = mulation(new_generation,mutate)
...
GenaticAlgorithm(populatin(100,5,0,100), 200)